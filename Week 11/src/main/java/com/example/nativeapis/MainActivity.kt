...
class MainActivity : AppCompatActivity() {
    ...
    private fun setupListeners() {
        binding.btnCall.setOnClickListener {
            if (isBatteryTooLowForCall()) {
                toast("Baterie critică! Apelul este blocat pentru a economisi energie.")
                log("BLOCAT: Încercare apel cu baterie < 15%.")
    ...
    private fun isBatteryTooLowForCall(): Boolean {
        val batteryManager = getSystemService(BATTERY_SERVICE) as BatteryManager
        val batteryLevel = batteryManager.getIntProperty(BatteryManager.BATTERY_PROPERTY_CAPACITY)

        log("Nivel baterie detectat: $batteryLevel%")
